...
describe 'azure_key_vault::secret' do
  ...
  context 'when passed the wrong arguments' do
    ...
    it 'errors when `cloud_type` is not a valid option' do
      ...
      bad_hash.delete('metadata_api_version')
      is_expected.to run.with_params(
        vault_name, secret_name, api_versions_hash.merge({ 'cloud_type' => 'invalid' })
  ...
  context 'when getting the latest version of a secret' do
    it 'defaults to using an empty string as the latest version' do
      expect(TragicCode::Azure).to receive(:get_access_token).with(api_versions_hash['metadata_api_version'], 'AzureCloud').and_return(access_token)
      expect(TragicCode::Azure).to receive(:get_secret).with(vault_name, secret_name, api_versions_hash['vault_api_version'], access_token, '', 'AzureCloud').and_return(secret_value)
  ...
  context 'when getting a specific version of a secret' do
    it 'uses the secret version when retreiving the secret' do
      expect(TragicCode::Azure).to receive(:get_access_token).with(api_versions_hash['metadata_api_version'], 'AzureCloud').and_return(access_token)
      expect(TragicCode::Azure).to receive(:get_secret).with(vault_name, secret_name, api_versions_hash['vault_api_version'], access_token, secret_version, 'AzureCloud').and_return(secret_value)
  ...
  context 'when getting a secret that does not exist in the vault' do
    it 'throws an error' do
      expect(TragicCode::Azure).to receive(:get_access_token).with(api_versions_hash['metadata_api_version'], 'AzureCloud').and_return(access_token)
      expect(TragicCode::Azure).to receive(:get_secret).with(vault_name, secret_name, api_versions_hash['vault_api_version'], access_token, secret_version, 'AzureCloud').and_return(nil)
  ...
  it 'retrieves access_token from cache' do
    expect(TragicCode::Azure).to receive(:get_access_token).with(api_versions_hash['metadata_api_version'], 'AzureCloud').and_return(access_token).once
    expect(TragicCode::Azure).to receive(:get_secret).with(vault_name, secret_name, api_versions_hash['vault_api_version'], access_token, '', 'AzureCloud').and_return(secret_value).twice
  ...
  context 'authenticating with service principal' do
    ...
    it 'returns the secret' do
      expect(TragicCode::Azure).to receive(:get_access_token_service_principal).with(api_versions_hash['service_principal_credentials'], 'AzureCloud').and_return(access_token)
      expect(TragicCode::Azure).to receive(:get_secret).with(vault_name, secret_name, api_versions_hash['vault_api_version'], access_token, '', 'AzureCloud').and_return(secret_value)
