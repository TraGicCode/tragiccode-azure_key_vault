...
describe TragicCode::Azure do
  context '.get_access_token' do
    it 'returns a bearer token' do
      ...
      expect(TragicCode::Azure.get_access_token('api')).to eq('token')
    end
    it 'errors when the response is not 2xx' do
      stub_request(:get, /169.254.169.254/).
        to_return(body: 'some_error', status: 400)
      expect { TragicCode::Azure.get_access_token('api') }.to raise_error('some_error')
    end
  end
  ...
  context '.get_secret' do
    it 'returns a secret' do
      ...
      expect(TragicCode::Azure.get_secret(vault_name, 'secret_name', 'api', 'token', '')).to eq('secret')
    end
    it 'errors when the response is not 2xx' do
      vault_name = 'vault'
      stub_request(:get, /vault/).
        to_return(body: 'some_error', status: 400)
      expect { TragicCode::Azure.get_secret(vault_name, 'secret_name', 'api', 'token', '') }.to raise_error('some_error')
    end
  end
end
